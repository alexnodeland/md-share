// Pure sanitizer for SVG rendered by @viz-js/viz (Graphviz) plus an
// error-box renderer. Graphviz emits declarative SVG — no scripts expected
// — but clickable nodes can emit <a xlink:href>, so we gate URLs the same
// way we do for abcjs and Vega-Lite output.
...
// Strip anything script-ish from Graphviz SVG output. Uses the global
...
export const sanitizeGraphvizSvg = (svg: string): string => {
...
export const renderGraphvizError = (message: string, source: string): string => {
  const msg = escapeXml(message || 'Invalid Graphviz DOT');
  const src = escapeXml(source.trim());
  return (
    `<div class="render-error"><strong>Graphviz render failed</strong>` +
    `<pre>${msg}</pre>` +
